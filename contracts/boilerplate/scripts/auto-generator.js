...
class CompactCLIAutoGenerator {
  ...
  async generateCLI(reason) {
    ...
    try {
      ...
      console.log(
        `📋 Found ${contractInfo.functions.length} functions and ${Object.keys(contractInfo.ledgerState).length} state variables`
      );
  ...
  async syncFromRoot() {
    ...
    const rootCompactFiles = rootFiles.filter((file) =>
    ...
    console.log(
      `📋 Found ${rootCompactFiles.length} .compact file(s) in root: ${rootCompactFiles.join(", ")}`
    );
    ...
    const existingCompactFiles = existingFiles.filter((file) =>
      file.endsWith(".compact")
    );

    if (existingCompactFiles.length > 0) {
      console.log(
        `🧹 Cleaning up ${existingCompactFiles.length} existing .compact file(s): ${existingCompactFiles.join(", ")}`
      );
    ...
    for (const compactFile of rootCompactFiles) {
      ...
      try {
        ...
        if (!content.includes("pragma language_version")) {
          console.warn(
            `⚠️  ${compactFile}: Missing pragma language_version directive`
          );
        }
    ...
    if (copiedFiles.length > 0) {
      console.log(
        `🎉 Successfully synced ${copiedFiles.length} contract file(s) from root to contract/src/`
      );
      return true;
    }
  ...
  validateEnvironment() {
    ...
    if (!fs.existsSync(path.resolve(process.cwd(), "package.json"))) {
      errors.push(
        "❌ No package.json found. Make sure you're in the project root directory."
      );
    }
    ...
    if (!fs.existsSync(path.resolve(process.cwd(), "boilerplate"))) {
      errors.push(
        "❌ Boilerplate directory not found. This doesn't appear to be a scaffold-midnight project."
      );
    }
    ...
    try {
      ...
      errors.push(
        "❌ Compact compiler (compactc) not found. Please install the Midnight development tools."
      );
    }
  ...
  detectContractFile() {
    ...
    if (compactFiles.length > 1) {
      console.log(
        `⚠️  Found ${compactFiles.length} .compact files: ${compactFiles.join(", ")}`
      );
  ...
  async parseContract() {
    ...
    if (!contractFileName) {
      ...
      const explicitPath = path.join(
        this.config.contractSourceDir,
        contractFileName
      );
      if (!fs.existsSync(explicitPath)) {
        console.log(
          `⚠️  Specified contract file not found: ${contractFileName}`
        );
        contractFileName = this.detectContractFile();
        this.config.contractFileName = contractFileName;
      }
...
export class EnhancedContractAPI {
  ...
  // Dynamic function mapping based on contract analysis${contractInfo.functions
    .map(
      (func) => `
  /**
...
`;
  ...
  async updateCoreAPI(contractInfo) {
    ...
    if (
      !apiContent.includes("const contractModule = ") &&
      !apiContent.includes("getContractModule")
    ) {
      const importSection = apiContent.match(
        /(import[\s\S]*?from[^;]+;)\s*\n/g
      );
      if (importSection) {
...
`;
        apiContent =
          apiContent.slice(0, insertPoint) +
          dynamicHelperCode +
          apiContent.slice(insertPoint);
      }
    }
...
class CompactContractParser {
  parse(content, fileName) {
    ...
    const circuitRegex =
      /export\s+circuit\s+(\w+)\s*\(([^)]*)\)\s*:\s*([^{]+)\s*\{/g;
    while ((match = circuitRegex.exec(content)) !== null) {
      ...
      const readOnly =
        (returnType.trim() !== "[]" && returnType.trim() !== "") ||
